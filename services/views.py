...
def recent_manual_services():
    return Service.objects.filter(metadata__source="dashboard_manual").order_by("-id")[:10]
...
GIS_LAYERS = {
    ...
    "village_nearest_police_station_line": {
        "schema": "final_api",
        "table": "village_nearest_police_station_line",
        "geom": "geom_4326",
        "exclude": ["geom", "geom_4326"],
    },
    "village_nearest_railway_station_line": {
        "schema": "final_api",
        "table": "village_nearest_railway_station_line",
        "geom": "geom_4326",
        "exclude": ["geom", "geom_4326"],
    },
    "village_nearest_bus_stand_line": {
        "schema": "final_api",
        "table": "village_nearest_bus_stand_line",
        "geom": "geom_4326",
        "exclude": ["geom", "geom_4326"],
    },
    "sector_nearest_police_station_line": {
        "schema": "final_api",
        "table": "sector_nearest_police_station_line",
        "geom": "geom_4326",
        "exclude": ["geom", "geom_4326"],
    },
    "sector_nearest_railway_station_line": {
        "schema": "final_api",
        "table": "sector_nearest_railway_station_line",
        "geom": "geom_4326",
        "exclude": ["geom", "geom_4326"],
    },
    "sector_nearest_bus_stand_line": {
        "schema": "final_api",
        "table": "sector_nearest_bus_stand_line",
        "geom": "geom_4326",
        "exclude": ["geom", "geom_4326"],
    },
    "chandigarh_boundary": {
...
def route_cache_api(request):
    """
    Saved Routes / Route History API.
    Only last 3 routes will be returned with start/end point labels.
    """
    import json
    from django.db import connection
    ...
    for row in rows:
        ...
        features.append({
            ...
            "properties": {
                ...
                "cost_by": cost_by,
                "origin_label": origin_label or "Start Point",
                "destination_label": destination_label or "End Point",
...
def village_nearest_police_station_api(request):
    return gis_layer_api(request, "village_nearest_police_station_line")


def village_nearest_railway_station_api(request):
    return gis_layer_api(request, "village_nearest_railway_station_line")


def village_nearest_bus_stand_api(request):
    return gis_layer_api(request, "village_nearest_bus_stand_line")


def sector_nearest_police_station_api(request):
    return gis_layer_api(request, "sector_nearest_police_station_line")


def sector_nearest_railway_station_api(request):
    return gis_layer_api(request, "sector_nearest_railway_station_line")


def sector_nearest_bus_stand_api(request):
    return gis_layer_api(request, "sector_nearest_bus_stand_line")

def login_live_stats_api(request):
    from django.http import JsonResponse
    from django.db import connection
    ...
    def safe_count(cursor, schema, table):
    ...
    def safe_road_km(cursor):
        schema = "routing"
        table = "roads_edges_routable"

        if not table_exists(cursor, schema, table):
            return 0

        where_sql = ""
        if column_exists(cursor, schema, table, "passable"):
            where_sql = "WHERE passable = true"

        if column_exists(cursor, schema, table, "length_m"):
            sql = '''
                SELECT ROUND((COALESCE(SUM(length_m), 0) / 1000.0)::numeric, 0)
                FROM "{}"."{}"
                {};
            '''.format(schema, table, where_sql)
        else:
            sql = '''
                SELECT ROUND((COALESCE(SUM(ST_Length(geom)), 0) / 1000.0)::numeric, 0)
                FROM "{}"."{}"
                {};
            '''.format(schema, table, where_sql)
    ...
    with connection.cursor() as cursor:
        govt_hospitals = safe_count(cursor, "final_api", "govt_hospitals")
        private_hospitals = safe_count(cursor, "final_api", "private_hospitals")
        schools = safe_count(cursor, "final_api", "schools")
        colleges = safe_count(cursor, "final_api", "colleges")
        villages = safe_count(cursor, "final_api", "villages_demand")
        road_network_km = safe_road_km(cursor)

    return JsonResponse({
        "success": True,
        "healthcare": govt_hospitals + private_hospitals,
